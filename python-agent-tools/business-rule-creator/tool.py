    ...
    def invoke(self, input, trace):
        ...
        try:
            ...
            if not python_condition:
                return {
                    "output": json.dumps(
                        {"success": False, "error": "python_condition is required"}
                    )
                }
            ...
            if action == "analyze":
                ...
                human_readable = self._convert_to_human_readable(
                    python_condition, columns_used
                )
                    ...
                    "output": json.dumps(
                        {
                            ...
                            "impact": {
                                ...
                                "violation_percentage": round(
                                    impact["violation_percentage"], 2
                                ),
                                "passing_percentage": round(
                                    impact["passing_percentage"], 2
                                ),
                            },
                            "columns_used": columns_used,
                            "python_condition": python_condition,
                            "human_readable_condition": human_readable,
                            "guidance": self._get_impact_guidance(
                                impact["violation_percentage"]
                            ),
                        }
                    )
                }
            ...
            return {"output": json.dumps({"success": False, "error": str(e)})}
    ...
    def _analyze_impact(self, dataset, python_condition, columns_used):
        ...
        try:
            ...
            if not hasattr(violating_mask, "sum"):
                raise ValueError(
                    "Condition did not return a pandas Series/Boolean mask"
                )
            ...
            return {
                ...
                "violation_percentage": (
                    (violating_rows / total_rows * 100) if total_rows > 0 else 0
                ),
                "passing_percentage": (
                    (passing_rows / total_rows * 100) if total_rows > 0 else 0
                ),
            }
        except Exception as e:
    ...
    def _create_dq_rule(
        ...
        try:
            ...
            rule_config = {
                ...
                "envSelection": {"envMode": "USE_BUILTIN_MODE", "envName": ""},
                "meta": {"label": business_rule, "name": rule_name_with_tag},
            }
